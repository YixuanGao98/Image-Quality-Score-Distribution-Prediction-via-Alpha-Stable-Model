function feat = MultiplicativeNoise(img)
if size(img,3)==3
...
for i = 1:length(noise)
    rng(seed);
    imgPRI = imnoise(img,'speckle',noise(1,i));
    if size(imgPRI,3)==3
    ...
    LBP(i+1,:)=LBP(i+1,:)./(m*n);
css(i)=sum(((LBP(i+1,:)-LBP(1,:)).^2)./(LBP(i+1,:)+LBP(1,:)));%卡方
feat(1,i) =css(i);
end
end
...
% feat(1,i) = sum(sum(mapDIST.*mapPRI))/(sum(summapDIST))+1);
% end
% 
% end
